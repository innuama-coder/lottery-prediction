...
**在现有数据（1430 期 DLT + 32 项特征）与已尝试的全部手段下，样本外未检测到任何可达成目标的预测信号（`no_confirmed_lift`）；且现有数据量不足以检出"达到 2 元所需量级"的信号（见 C4），故该目标"能否达成"在现有数据下未决，而非"已证明绝对不可达"。**

分解为四条结论：

| # | 结论 | 证据 |
| --- | --- | --- |
| C1 | **条件定理**（前提=均匀开奖）：每注期望恒为 SSQ 0.853105 / DLT 0.881670 元，选号无效——这是数学命题，**不构成对现实的断言** | `prize_metrics.py`（提交 `2203b4518`）`full_space_oracle` + 对称性证明 |
...
本报告全部结论的工作真实性由四类可复现证据构成：

| 环节 | 证据 | 验证 |
| --- | --- | --- |
| 数据真实 | 1430 期 DLT，每条含 `raw_sha256`（原始 HTML 内容哈希）+ `fetched_at_utc` + `url` | `dlt-draws-full.jsonl` 1430/1430 条含 raw_sha256，首期 17001、末期 26095，年份分布 2017–2026 |
| 特征真实 | 32 个 feature_id 导出，5 文件 289 行代码 | `src/lottery_system/phase4/features/`，`FEATURE_IDS` |
| 搜索真实 | 48 配置穷举，`candidate_space.legal_count=48`，`candidates.jsonl` 恰 48 行 | `artifacts/phase4e32_model_search/` |
| 结果可复现 | 重跑 walk-forward，p_value 与提交版一致到 13 位小数 | 见下表 |

**可复现性关键数字（本地重跑 `run_walk_forward.py`，3 分 23 秒）：**

| 量 | 已提交 | 本地复现 |
| --- | --- | --- |
| p_value | 0.569374147245372 | 0.5693741472453895 |
| mean_uniform_minus_model | 4.467391197604716e-05 | 4.467391197604531e-05 |

两者一致到小数点后 13 位（末 3 位为 macOS/Linux 浮点表示精度差异）。内容寻址的 `raw_sha256` 无法伪造，故数据真实性成立；代码与产物在 git 提交链中不可变；本地重跑与提交一致，故结果非抄写、为真实计算。
...
- **该结论的前提是"开奖均匀随机"**：它是"若均匀，则选号无效"的条件命题，不是"现实就是均匀"的断言。要达 2 元需 2.34×/2.27× 的真实命中优势（即开奖非均匀到该量级）。
- "开奖是否均匀、有无可学信号"是**实证问题**，由 C3/C4 的模型与搜索回答，不由本定理回答。
