...
def validate_schema_instance(instance: Any, schema: dict[str, Any], *, root: dict[str, Any] | None = None, path: str = "$") -> None:
    ...
    if "if" in schema:
        branch = "then" if _schema_matches(instance, schema["if"], root, path) else "else"
        if branch in schema:
    ...
    if isinstance(instance, str):
        ...
        if schema.get("format") == "date" and not _is_date(instance):
            raise ValidationError(f"{path}: expected a real YYYY-MM-DD calendar date")
        if "format" in schema and schema["format"] not in {"date-time", "date"}:
    ...
    def walk(node: dict[str, Any], path: str) -> None:
        ...
        for keyword in ("if", "then", "else"):
            if isinstance(node.get(keyword), dict):
                walk(node[keyword], f"{path}/{keyword}")
