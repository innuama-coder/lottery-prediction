...
- **Pre-allocation HOLD：** controller 报告的 coefficient 15 ULP 是 example。只有 macOS collector 实际覆盖所有 86 patterns、全部 coefficient/objective-gradient comparisons 且新 profile failures 为 0，才可把 16 ULP 当作本 release migration envelope；此前禁止分配 successor release。
