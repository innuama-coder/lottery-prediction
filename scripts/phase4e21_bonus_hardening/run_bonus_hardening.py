...
def state_space_audit() -> dict[str, object]:
    configurations = (
        ("ssq", SSQ_OLD_RULE, 7, 2, SSQ_TIER_STATES, SSQ_FIXED_PRIZES),
        ("ssq", SSQ_NEW_RULE, 7, 2, SSQ_TIER_STATES, SSQ_FIXED_PRIZES),
        ("dlt", DLT_OLD_RULE, 6, 3, DLT_OLD_TIER_STATES, DLT_OLD_FIXED_PRIZES),
        ("dlt", DLT_NEW_RULE, 6, 3, DLT_NEW_TIER_STATES, DLT_NEW_FIXED_PRIZES),
    )
    results = []
    ...
    return {
        ...
        "all_checks_pass": all(
            row["mutually_exclusive"] and row["all_amounts_integer_yuan"]
            for row in results
        ),
    }
...
def fast_ranked_ticket_partition_prize_metrics(
    ...
    tier_max = 6 if game == "ssq" else (9 if version == DLT_OLD_RULE else 7)
