...
def run_g2(contract: dict[str, Any], contract_path: Path) -> tuple[int, dict[str, Any]]:
    ...
    formal_before = _formal_state(exclude=frozenset({"acceptance/g2.json"}))
    environment = os.environ.copy()
    ...
    unchanged = formal_before == _formal_state(exclude=frozenset({"acceptance/g2.json"}))
...
G3_DERIVED_OUTPUTS = frozenset({"acceptance/g3.json", "acceptance/all.json"})
...
def _g3_input_inventory() -> dict[str, Any]:
    targets = {
        ...
        "acceptance/g1.json": FORMAL_ROOT / "acceptance" / "g1.json",
        "acceptance/g2.json": FORMAL_ROOT / "acceptance" / "g2.json",
...
def _parse_unittest_verbose(stderr: str) -> dict[str, Any]:
    ...
    valid = (
        ...
        and not re.search(r"skipped|expected failure|unexpected success|FAILED", stderr, re.I)
    )
...
def _evaluate_g3_assertions(
    ...
    for assertion_id in EXPECTED_G3_ASSERTIONS:
        ...
        if assertion_id in static:
            ...
            evidence = {"kind": "e2e_set", "cases": sorted(children), "g2_sha256": inventory["acceptance/g2.json"].get("sha256")}
...
def _g3_inputs(contract: dict[str, Any], contract_path: Path) -> tuple[list[str], dict[str, dict[str, Any] | None], list[str], bool]:
    g1_state, g1 = _dependency_report(FORMAL_ROOT / "acceptance" / "g1.json", "G1", contract, contract_path)
    g2_state, g2 = _dependency_report(FORMAL_ROOT / "acceptance" / "g2.json", "G2", contract, contract_path)
    ...
    if g1_state == g2_state == "valid" and isinstance(g1, dict) and isinstance(g2, dict):
        g1_path = FORMAL_ROOT / "acceptance" / "g1.json"
...
def run_g3(contract: dict[str, Any], contract_path: Path) -> tuple[int, dict[str, Any]]:
    ...
    if profile_error is not None:
        return FAIL, _base_report(
            ...
            input_inventory=_g3_input_inventory(), commands=[], e2e_children={}, assertions=[],
        )
    inventory_before = _g3_input_inventory()
    ...
    inventory_after = _g3_input_inventory()
...
def run_all(contract: dict[str, Any], contract_path: Path) -> tuple[int, dict[str, Any]]:
    ...
    inventory = _g3_input_inventory()
    inventory_sha = _inventory_digest(inventory)
    dependency_states, dependencies, review_states, binding_ok = _g3_inputs(contract, contract_path)
    g3_path = FORMAL_ROOT / "acceptance" / "g3.json"
    ...
    try:
        ...
        if args.execute_case:
            ...
            if gate == "G1":
                ...
                output = args.output or FORMAL_ROOT / "acceptance" / "g1.json"
            elif gate == "G2":
                expected_output = (FORMAL_ROOT / "acceptance" / "g2.json").resolve()
                ...
                expected_output = (FORMAL_ROOT / "acceptance" / "g3.json").resolve()
                if args.output is not None and args.output.resolve() != expected_output:
                    raise ValueError(f"G3 output must be {expected_output}")
                code, report = run_g3(contract, contract_path)
                output = expected_output
            elif gate == "ALL":
                expected_output = (FORMAL_ROOT / "acceptance" / "all.json").resolve()
                if args.output is not None and args.output.resolve() != expected_output:
