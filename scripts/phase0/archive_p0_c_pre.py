...
REPAIR_DRAFT_RELATIVE = "artifacts/phase-0/repair-manifest-p0-20260801-c-draft.json"
C_AFTER_BATCH_ID = "p0-20260801-c-after"
SUPERSEDED_C_AFTER_BATCH_ID = "p0-20260801-c-after-superseded-hardcoded-validation"
...
def historical_after_archive_root(repo_root: Path = ROOT) -> Path:
    """Resolve the immutable C-after generation available at this point in the lineage."""
    batches = repo_root / "artifacts/phase-0/batches"
    canonical = batches / C_AFTER_BATCH_ID
    if (canonical / "snapshot-manifest.json").is_file():
        return canonical
    superseded = batches / SUPERSEDED_C_AFTER_BATCH_ID
    if (superseded / "snapshot-manifest.json").is_file():
        return superseded
    return canonical


def historical_repair_draft_path(repo_root: Path = ROOT, archive_root: Path | None = None) -> Path:
    """Return the immutable archived draft; never fall back to mutable root state."""
    pre_archive = archive_root or repo_root / "artifacts/phase-0/batches" / BATCH_ID
    self_contained = pre_archive / REPAIR_DRAFT_RELATIVE
    if self_contained.is_file():
        return self_contained
    return historical_after_archive_root(repo_root) / REPAIR_DRAFT_RELATIVE


def verify_archived_repair_draft(
    repo_root: Path, draft_path: Path, *, pre_manifest_sha256: str,
    pre_snapshot: dict[str, Any], after_archive_root: Path | None = None,
) -> dict[str, Any]:
    """Verify the C-after archived draft record and its binding to C-pre."""
    after_archive = after_archive_root or historical_after_archive_root(repo_root)
    after_manifest_path = after_archive / "snapshot-manifest.json"
    after_sidecar_path = after_archive / "snapshot-manifest.json.sha256"
    if not after_manifest_path.is_file() or not after_sidecar_path.is_file():
        raise ArchiveError("C-after archive manifest or sidecar for historical repair draft is missing")
    if after_sidecar_path.read_text(encoding="ascii").strip() != sha256_file(after_manifest_path):
        raise ArchiveError("C-after snapshot manifest self-hash sidecar mismatch")
    after_snapshot = load_json(after_manifest_path)
    record = manifest_index(after_snapshot).get(REPAIR_DRAFT_RELATIVE)
    if record is None:
        raise ArchiveError("C-after snapshot does not record the historical repair draft")
    if not draft_path.is_file() or file_record(draft_path, REPAIR_DRAFT_RELATIVE) != record:
        raise ArchiveError("archived repair draft differs from C-after manifest")
    draft = load_json(draft_path)
    validate_repair_draft(draft, pre_snapshot, pre_manifest_sha256)
    return draft


def validate_repair_draft(draft: dict[str, Any], snapshot: dict[str, Any], snapshot_sha256: str) -> None:
    ...
    if draft.get("parent_lineage", {}).get("before_snapshot_manifest_sha256") != snapshot_sha256:
        raise ArchiveError("repair draft does not bind the C-pre snapshot manifest")
...
def verify_archive(
    repo_root: Path = ROOT, *, check_current: bool = False,
    archive_root: Path | None = None,
) -> dict[str, Any]:
    archive_root = archive_root or repo_root / "artifacts/phase-0/batches" / BATCH_ID
    snapshot_manifest = archive_root / "snapshot-manifest.json"
    snapshot_sidecar = archive_root / "snapshot-manifest.json.sha256"
    if not snapshot_manifest.is_file() or not snapshot_sidecar.is_file():
        raise ArchiveError("C-pre snapshot manifest or sidecar is missing")
    ...
    records = verify_snapshot_tree(snapshot, archive_root)
    for relative, record in records.items():
        if check_current:
    ...
    if check_current:
        draft_path = repo_root / REPAIR_DRAFT_RELATIVE
        if not draft_path.is_file():
            raise ArchiveError("current before-repair draft is missing")
        draft = load_json(draft_path)
        validate_repair_draft(draft, snapshot, expected_sidecar)
    return snapshot
