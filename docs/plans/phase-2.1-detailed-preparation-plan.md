...
1. 核对任务分支、目标分支基线 SHA、独立 worktree、原任务输入及 iteration-01/02；通过显式 `--task-input-dir` 校验三个文件的合同哈希，只复制输入快照，不修改任务目录与既有 runs。
...
7. 同一 release 内按 qualification、完整重算 audit、五 family/240 格完整 power、不同 seed 完整 replay、独立重放复核和 10 个 E2E 的顺序生成新证据。旧 Phase 2 结果 JSON 不作为本轮行来源；所有核心 JSON 立即通过专用 Schema。
