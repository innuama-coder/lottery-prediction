...
class P006RuntimeTests(unittest.TestCase):
    ...
    def test_installer_default_verify_is_read_only(self) -> None:
        ...
        completed = subprocess.run(["powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", str(SCRIPTS / "install_p0_06_scheduled_task.ps1")], cwd=REPO, capture_output=True, text=True, check=False, timeout=30)
        self.assertEqual(completed.returncode, 0, completed.stdout + completed.stderr)
