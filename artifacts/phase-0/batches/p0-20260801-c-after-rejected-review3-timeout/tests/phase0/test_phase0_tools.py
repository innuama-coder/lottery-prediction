...
from verify_phase0 import (  # noqa: E402
    AUXILIARY_SCHEMAS, FINALIZE_COMMAND, FULL_VERIFY_COMMAND, REPLAY_COMMAND,
    SCHEMA_BY_KEY, VERIFIER_FILES, verify_command, verify_observation,
    verify_reviewer, verify_scope,
)
...
class SchemaSubsetTests(unittest.TestCase):
    ...
    def test_unsupported_assertion_keyword_rejected(self) -> None:
        ...
        bad["contains"] = {"type": "object"}
        with self.assertRaisesRegex(ValidationError, "unsupported keywords"):
            validate_schema_instance({}, bad)

    def test_if_then_is_enforced(self) -> None:
        conditional = {
            "if": {"properties": {"state": {"const": "verified"}}},
            "then": {"required": ["available_at"], "properties": {"available_at": {"type": "string"}}},
        }
        validate_schema_instance({"state": "unverified"}, conditional)
        with self.assertRaisesRegex(ValidationError, "missing required"):
            validate_schema_instance({"state": "verified"}, conditional)
...
class VerificationCommandFailureInjectionTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.command.update({
            ...
            "finalize_launcher_sha256": sha256_file(REPO / "scripts/phase0/p0_07_finalize_launcher.ps1"),
            "finalizer_path": "scripts/phase0/p0_07_finalize.py",
            "finalizer_sha256": sha256_file(REPO / "scripts/phase0/p0_07_finalize.py"),
        })
        cls.command["verifier_file_hashes"] = [
            {"path": path, "sha256": sha256_file(REPO / path)} for path in sorted(VERIFIER_FILES)
        ]
        expected_names = set(SCHEMA_BY_KEY.values()) | AUXILIARY_SCHEMAS
        schema_dir = REPO / "artifacts/phase-0/schemas"
        cls.command["schema_hashes"] = [
            {"path": f"artifacts/phase-0/schemas/{name}", "sha256": sha256_file(schema_dir / name)}
            for name in sorted(expected_names)
        ]
        cls.command["schemas_manifest_sha256"] = schemas_manifest_sha256(schema_dir)
    ...
    def test_verifier_file_hash_tamper_is_rejected(self) -> None:
        ...
        schema_dir = REPO / "artifacts/phase-0/schemas"
        expected_names = set(SCHEMA_BY_KEY.values()) | AUXILIARY_SCHEMAS
        tampered["schema_hashes"] = [
            {"path": f"artifacts/phase-0/schemas/{name}", "sha256": sha256_file(schema_dir / name)}
            for name in sorted(expected_names)
        ]
        tampered["schemas_manifest_sha256"] = schemas_manifest_sha256(schema_dir)
        with self.assertRaisesRegex(ValidationError, "verification tool hash mismatch"):
            verify_command(REPO, REPO / "docs/roadmap/phase-0-acceptance-contract.json", REPO / "artifacts/phase-0", REPO / "artifacts/phase-0/schemas", tampered)
...
class EndToEndP001Tests(unittest.TestCase):
    def test_frozen_p0_01_accepts_the_complete_c_repair_command(self) -> None:
        ...
        command = load_json(REPO / "artifacts/phase-0/verification-command.json")
        self.assertEqual(command["freeze_id"], "P0-01-p0-20260801-c-repair")
        self.assertEqual(len(command["schema_hashes"]), 32)
        self.assertEqual(len(command["verifier_file_hashes"]), 23)
        self.assertEqual(command["command"], command["full_replay_command"])
        self.assertEqual(command["command"], command["replay_command"])
        self.assertEqual(len({command["replay_command"], command["finalize_command"], command["full_verify_command"]}), 3)
