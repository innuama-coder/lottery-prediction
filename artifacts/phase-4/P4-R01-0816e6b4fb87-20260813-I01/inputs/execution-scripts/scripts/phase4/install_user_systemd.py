...
def main()->int:
 ...
 actor=r/'control/actor-assignments-formal.json';assignment=json.loads(actor.read_text());vps=[x for x in assignment['assignments'] if 'vps_operator' in x['roles']][0];envfile=runtime/'runtime-provenance.env';runtime.mkdir(parents=True,exist_ok=True);schedule=r/'tests/phase4/fixtures/schedule/dual-game.json'
 envfile.write_text(f"P4_PROJECT_ROOT={r}\nP4_RUNTIME_ROOT={runtime}\nP4_ACTOR_ID={vps['actor_id']}\nP4_SESSION_ID={vps['session_id']}\nP4_TASK_ID=T18\nP4_ROLE=vps_operator\nP4_ACTOR_ASSIGNMENTS={actor.relative_to(r)}\n",encoding='utf-8')
