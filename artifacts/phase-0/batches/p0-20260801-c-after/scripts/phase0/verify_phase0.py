...
AUXILIARY_SCHEMAS = {
    ...
    "p0-07-derived-manifest.schema.json",
    "p0-07-gate-inputs.schema.json",
    "p0-07-input-manifest.schema.json",
    "p0-07-stage1-consumer-receipt.schema.json",
    "p0-07-terminal-manifest.schema.json",
    "p0-07-bundle-manifest.schema.json",
    "p0-07-content-manifest.schema.json",
    "p0-07-execution-receipt.schema.json",
    "technical-replay-report.schema.json",
    "p0-07-reviewer-verification-receipt.schema.json",
    "machine-acceptance-decision.schema.json",
}
...
REPLAY_COMMAND = "powershell -NoProfile -ExecutionPolicy Bypass -File scripts/phase0/p0_07_replay_launcher.ps1"
FINALIZE_COMMAND = "powershell -NoProfile -ExecutionPolicy Bypass -File scripts/phase0/p0_07_finalize_launcher.ps1"
VERIFIER_FILES = {
    ...
    "scripts/phase0/p0_05_history.py",
    "scripts/phase0/p0_07_closeout.py",
    "scripts/phase0/p0_07_decision.py",
    "scripts/phase0/p0_07_handoff.py",
    "scripts/phase0/p0_07_replay_launcher.ps1",
    "scripts/phase0/p0_07_replay_driver.py",
    "scripts/phase0/p0_07_clean_worker.py",
    "scripts/phase0/p0_07_stage1_consumer.py",
    "scripts/phase0/p0_07_replay_model.py",
    "scripts/phase0/p0_07_reviewer_verify.py",
    "scripts/phase0/p0_07_reviewer_attest.py",
    "scripts/phase0/p0_07_final_model.py",
    "scripts/phase0/p0_07_finalize.py",
    "scripts/phase0/p0_07_finalize_launcher.ps1",
}
...
def verify_command(repo_root: Path, contract_path: Path, artifacts: Path, schema_dir: Path, command: dict[str, Any]) -> None:
    ...
    fail(command["command"] != REPLAY_COMMAND or command["full_replay_command"] != REPLAY_COMMAND or command["replay_command"] != REPLAY_COMMAND, "replay command fields must be the one fixed replay launcher")
    fail(command["finalize_command"] != FINALIZE_COMMAND, "finalize_command is not canonical")
    fail(command["full_verify_command"] != FULL_VERIFY_COMMAND, "full_verify_command is not canonical")
    fail(len({command["replay_command"],command["finalize_command"],command["full_verify_command"]})!=3,"replay, finalize, and full verify commands must be distinct")
    ...
    for path_field, hash_field in (("finalize_launcher_path","finalize_launcher_sha256"),("finalizer_path","finalizer_sha256")):
        path=repo_path(repo_root,command[path_field])
        fail(not path.is_file() or sha256_file(path)!=command[hash_field],f"terminal tool hash mismatch: {command[path_field]}")
    planned = {
...
def verify_p0_06_semantics(
    ...
    *,
    ...
    cutoff = _parse_audit_time(plan["acceptance_cutoff_utc"], "acceptance_cutoff_utc").astimezone(timezone.utc)
    if require_complete:
        evaluated_at = (verified_at_utc or datetime.now(timezone.utc)).astimezone(timezone.utc)
        fail(evaluated_at < cutoff, "P0-06 full completion gate cannot run before the frozen acceptance cutoff")
    seen: set[str] = set()
    evidence_manifest: dict[str, dict[str, Any]] = {}
    ...
    for entry in entries:
        ...
        if blocked:
            ...
            fail(_parse_audit_time(entry["started_at_utc"], f"{request_id} started_at_utc").astimezone(timezone.utc) > cutoff, f"{request_id}: network was used after the frozen acceptance cutoff")
    ...
    fail(sum(int(entry["attempts"]) for entry in entries) > len(authorized), "P0-06 network-attempt budget exceeds the six frozen authorized requests")
    fail(len(entries) > observation["budgets"]["total_request_limit"], "P0-06 logical request count exceeds the frozen total request budget")
    if require_complete:
        fail(seen != set(requests) or len(entries) != len(requests), "P0-06 full completion gate requires each of the 24 frozen requests exactly once")
...
def verify_p0_06_install_audit_semantics(
    ...
    snapshot_root = artifacts / "batches" / "p0-20260801-c-pre"
    snapshot_path = snapshot_root / "snapshot-manifest.json"
    snapshot_sidecar = snapshot_root / "snapshot-manifest.json.sha256"
    fail(not snapshot_path.is_file() or not snapshot_sidecar.is_file(), "P0-06 install audit historical C-pre anchor is missing")
    snapshot_hash = snapshot_sidecar.read_text(encoding="ascii").strip()
    fail(re.fullmatch(r"[0-9a-f]{64}", snapshot_hash) is None or snapshot_hash != sha256_file(snapshot_path), "P0-06 C-pre snapshot manifest sidecar mismatch")
    snapshot = load_json(snapshot_path)
    snapshot_files = {item["path"]: item for item in snapshot["files"]}

    def anchored(relative: str) -> dict[str, Any]:
        fail(relative not in snapshot_files, f"P0-06 C-pre snapshot lacks historical anchor: {relative}")
        record = snapshot_files[relative]
        archived = snapshot_root / Path(relative)
        fail(not archived.is_file() or sha256_file(archived) != record["sha256"] or archived.stat().st_size != record["size"], f"P0-06 archived historical bytes mismatch: {relative}")
        return record

    audit_anchor = anchored("artifacts/phase-0/p0-06-scheduler-install-audit.json")
    evidence_anchor = anchored("artifacts/phase-0/evidence-manifest.jsonl")
    soak_anchor = anchored("artifacts/phase-0/soak-run-log.jsonl")
    fail(not audit_path.is_file() or sha256_file(audit_path) != audit_anchor["sha256"], "P0-06 install audit differs from its immutable C-pre historical anchor")
    ...
    fail(audit["soak_log_bytes"] != 0 or soak_anchor["size"] != 0 or soak_anchor["sha256"] != sha256_bytes(b""), "P0-06 install audit historical anchor does not prove an initially empty soak log")
    fail(audit["evidence_manifest_sha256"] != evidence_anchor["sha256"], "P0-06 install audit evidence SHA-256 differs from its historical anchor")
    fail(not evidence_path.is_file(), "P0-06 current evidence manifest is missing")
    archived_evidence = (snapshot_root / "artifacts/phase-0/evidence-manifest.jsonl").read_bytes()
    current_evidence = evidence_path.read_bytes()
    fail(not current_evidence.startswith(archived_evidence), "P0-06 current evidence manifest is not an append-only extension of the installed-state anchor")
...
def verify_full(contract: dict[str, Any], repo_root: Path, contract_path: Path, artifacts: Path, schema_dir: Path) -> None:
    ...
    verify_p0_06_semantics(runtime_plan, validated["soak_log"], load_json(artifacts / "observation-plan.json"), validated["source_catalog"], repo_root, require_complete=True)
    ...
    for game, result in results.items():
        ...
        expected_coverage = {"PASS_FULL": "target", "PASS_LIMITED": "minimum_viable"}.get(result["per_game_outcome"])
        fail(expected_coverage is not None and result["coverage_tier"] != expected_coverage, f"{game}: coverage tier contradicts outcome")
    ...
    from p0_07_finalize import verify_terminal
    verify_terminal(repo_root=repo_root, artifacts=artifacts, require_sidecar=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 0
    except (ValidationError, ValueError, KeyError, OSError, UnicodeError) as exc:
