...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify the immutable manifest and all allowed W12-W13 lifecycle outputs."
    )
    ...
    frozen_checks = load(release / "handoff-validation/frozen-acceptance-checks-through-W13/summary.json")
    commands = frozen_checks.get("commands", [])
    if (
        frozen_checks.get("status") != "PASS"
        or frozen_checks.get("command_count") != 18
    ...
    for ordinal in range(7, 14):
        receipt = load(release / f"work-items/W{ordinal:02d}/receipt.json")
        if (
    ...
    readiness_revalidation = load(
        release / "handoff-validation/readiness-revalidation/readiness-revalidation.json"
    )
    ...
    for name, artifact in (
        ...
        ("READINESS_REVALIDATION", readiness_revalidation),
        ("HANDOFF_VALIDATION", handoff_validation),
    ):
        if artifact.get("status") != "PASS" or artifact.get("blocking_findings") not in (None, 0, []):
            blockers.append(name)

    guarded = load(release / "review/w12-guarded-unlock-recomputation.json")
    guarded_expected = {
        "status": "PASS",
        "expected_guarded_unlock_count": 600,
        "guarded_unlock_count": 600,
        "unique_unlock_receipt_count": 600,
        "unlock_receipt_file_count": 600,
        "pre_lock_label_read_count": 0,
        "identity_or_hash_mismatch_count": 0,
        "ledger_identity_or_sequence_mismatch_count": 0,
        "receipt_inventory_mismatch_count": 0,
    }
    for key, value in guarded_expected.items():
        if guarded.get(key) != value:
            blockers.append(f"GUARDED_UNLOCK:{key}")
