...
T13 + T14 -> T15 benchmark, resource/seed/identity freeze and formal release
 ...
 -> T23 machine delivery statement
...
- **前置输入及固定身份：** controller 先 `git fetch origin main`，再显式提供在本版设计/计划合入后选定的 `P4_AUTHORITY_COMMIT`；该提交必须是 `origin/main` 祖先，同时包含 SHA-256 分别为 `24ba28e72c33959a91e505fd518718bd0c948c84b7e2e4cd5591a26f0a0b0149`、`aa3221652e2e0b9876f233b37191267d3be7f0f56cd0d0e82481dcf1c0aeb874` 的 `ROADMAP.md`、`tasks/phase4/README.md`，并同时含有本文和 `docs/research/phase-4-overall-design.md`。四文件的内容 SHA 从该 Git 对象在 T00 receipt 中冻结；不把文档自哈希写入文档本身。另固定 Phase 3 `P3-R07-2c0fa97-20260810-I01` acceptance SHA-256 `415bfc69cc04704265e231fd7d6e36bd2daa06b970b0def30703c4a7f04570c9` 和 Phase 1 四项 genesis。所有 preparation actor、完整不等式以及机器交付声明器和终局机器验收角色必须在运行前给出。无任务依赖。
...
- **独立验收标准与方法：** data custodian 从 Git 对象和文件字节重算所有 SHA、六个受保护根的全路径集合及 Phase 1 四项内容；机器确认 authority commit 属于 `origin/main`、两份上位合同 hash 精确匹配、设计与计划两路径均存在且 receipt 中的四文件 hash 全部由同一提交重算一致、机器交付声明器和终局机器验收角色已固定且 actor 不等式成立；四个 Phase 4 根 realpath 互不嵌套、ID 不复用且无 `latest`；路径/哈希/计数/角色绑定 100%，dirty 修改为 0。负测分支-only commit、只含相同上位合同但仍是旧版设计/计划的 commit、换 commit、空 baseline、改一字节、任一受保护根删/加文件、缺机器角色、角色复用、`latest`、prep 写入 staging/runtime/formal。
- **失败终态/证据/取回：** 未合入 main 为 `HOLD_AUTHORITY_NOT_ON_MAIN`，身份/声明缺失为 `HOLD_AUTHORITY_IDENTITY|HOLD_MACHINE_DELIVERY_STATEMENT`，上游篡改为 `FAIL_PROTECTED_ARTIFACT_MUTATION`；证据在 `$P4_PREP_ROOT/work-items/T00/`，按 receipt 显式清单取回并逐哈希。
...
- **目标/执行角色：** `implementation_author` 只实现已经由 T01 的 `statistical_owner` 声明冻结的控制器和修订闭包第二段 research remediation；该任务只拥有 research control plane，不能在实现时重新决定统计规则。
- **前置输入及固定身份：** T01 model/feature/decision/alpha contracts及 statistical-owner receipt；T02 ledger/checkpoint；T06 current scores/correction hold port。
...
- **交付物及接口：** 8 个 cell 每个恰好 20,000 个独立 sequence terminals，共 160,000 个序列和 24,000,000 个 draw observations；按 T12 批准的无损分片合同保存底层 draws/terminals/provenance；sequence success count/rate 和 Bonferroni simultaneous 95% Clopper–Pearson `[L,U]`；prospective aggregate point/interval；seed-set hashes/intersection=0；analytic comparison；full-rule spec/oracle expected values；`qualification-design.json` 声明。字段必须分名为 `sequence_rate_estimate`、`sequence_rate_simultaneous_interval`、`formal_1000_gate_pass_probability_estimate`、`formal_1000_gate_pass_probability_interval`，禁止把 rate 写成 gate probability。
...
- **目标/执行角色：** `release_controller` 只消费 T13 已声明 PASS design，续验 authority、封装自包含 preparation evidence，并把正式 benchmark 代入固定公式，冻结批准 workload、formal actor assignment、formal seed、code/input/contracts/dependencies 和唯一空 release；这是正式结果前最后门，不能重新选择 design。
- **前置输入及固定身份：** T00–T14 全 PASS；T00 `P4_AUTHORITY_COMMIT` 及四文件 blob inventory；T01 contract-bundle inventory；T13 signed design、完整 development/power manifests、两个 uniform 的 sequence error-rate intervals、六个 positive 的 sequence recovery-rate intervals 和八个 aggregate probability intervals；T14 wheelhouse/product wheel manifest及 `built_from_commit`；当前 clean `P4_IMPLEMENTATION_COMMIT`；T00 全量 protected inventory、完整正式 provenance-derived actor 不等式及已确认的机器交付声明者；formal root 和 `$P4_RELEASE_VENV` 原先均不存在且 formal terminal count 为 0。
...
- **目标/执行角色：** `independent_replay_operator` 只执行已冻结的独立 replay；不 review、不运行 final validator、不声明或 acceptance。
- **前置输入及固定身份：** T19 evidence manifest SHA、T15 actor contract、同一 frozen release；不以产品 summary 为真值。
- **允许修改/禁止修改：** 只允许 `$P4_RELEASE_ROOT/{replay,manifest/replay-closure.json,work-items/T20}`；禁止产品、既有 evidence、validator/review/delivery/acceptance。
...
- **目标/执行角色：** `acceptance_engineer` 只从 T19 evidence 与 T20 replay 执行 A01–A21 final validator；不 review、不声明、不写 acceptance。
...
- **目标/执行角色：** `independent_reviewer` 只审查 T21 validator、T20 replay、证据完整性和独立性；不运行产品、不修改 validator、不声明或 acceptance。
...
### T23：机器交付声明与科学措辞一致性检查

- **目标/执行角色：** 独立的 `machine_delivery_statement` actor 机器审阅 T22 提供的最终交付矩阵和科学措辞，检查没有把合成能力写成真实改善；不改任何证据。
- **前置输入及固定身份：** T22 review closure、T21 validator closure、T00/T15 machine delivery assignment，同一 release。
- **允许修改/禁止修改：** 只允许 `$P4_RELEASE_ROOT/{delivery/machine-delivery-statement.json,manifest/delivery-closure.json,work-items/T23}`；声明对象只能引用既有 hash 并记录 machine actor identity/time/decision/checks，不得生成技术事实。
- **交付物及接口：** 一份同时覆盖 delivery completeness 与 scientific wording 的机器交付声明和 machine-delivery closure；上位合同不要求任何人类 acknowledgement。
- **依赖/执行命令：** T22；先通过 execution preflight，由机器交付声明器使用冻结校验入口生成声明，随后运行 `"$P4_RELEASE_PYTHON" "$P4_FROZEN_SCRIPTS/scripts/phase4_independent/validate_machine_delivery_statement.py" --delivery-statement "$P4_RELEASE_ROOT/delivery/machine-delivery-statement.json" --review-closure "$P4_RELEASE_ROOT/manifest/review-closure.json" --actor-assignments "$P4_FORMAL_ACTORS"`。
- **独立验收标准与方法：** release controller 只验证声明 actor 已指派、与被声明 producer/reviewer 集合不相交、`decision=PASS`、引用 hash 精确且 actor 不等式成立；缺少检查、空白结论或引用旧 review 均失败。`acceptance_approver` 在 T23 不执行、不验声明、不生成 receipt，首次且仅在 T24 介入。
- **失败终态/证据/取回：** 缺少或拒绝声明为 `HOLD_MACHINE_DELIVERY_STATEMENT`；伪造为 FAIL。

### T24：独立最终交付验收（最后任务）

- **目标/执行角色：** 仅 `acceptance_approver` 对 T23 已闭合的同一冻结 release 从底层证据签发唯一工程终态；作者不得验收自己的实现。
- **前置输入及固定身份：** T23 machine-delivery closure、T22 review closure、T21 validator closure、T20 replay closure、T19 manifest、T15 acceptance/actor contracts，全部同 `P4_RELEASE_ID` 和固定 SHA。
...
- **失败终态/证据/取回：** 可恢复未完成 `HOLD`，不可恢复因果/篡改/伪造/越权 `FAIL`；不得写 `READY_FOR_HUMAN_ACCEPTANCE`。失败 I01 不覆盖，修复按第 6 节用新 iteration/release。证据从 `$P4_RELEASE_ROOT/acceptance/I01/` 和 T24 receipt 取回。

## 4. A01–A21 双向追踪矩阵

命令简称：`U=unittest tests/phase4`，`E=validate e2e`，`Q=T16 formal qualification`，`O=T10/T16 independent oracle`，`C=T03 early + T17 formal canary`，`S=T08 early probe + T18 schedule audit`，`R=T20 replay`，`V=T21 final validator`，`W=T22 review + T23 machine delivery statement`，`A=T24 accept`。所有简称均指上面任务卡的完整命令和对应 receipt。
...
| P4-MVP-A16 | 14 | T01,T19,T20,T21,T22,T23,T24 | coverage/replay/validator/review/machine delivery/acceptance | V,W,A |
