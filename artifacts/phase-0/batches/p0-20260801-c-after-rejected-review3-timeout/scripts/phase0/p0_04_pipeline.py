...
def rebuild_captures(artifacts: Path, clock_check: ClockCheck, output_root: Path) -> list[Path]:
    """Rebuild parsed/normalized outputs from raw+manifest without reading stored derived files."""
    artifacts = artifacts.resolve()
    output_root = output_root.resolve()
    if not clock_check.passed:
        raise ValidationError("capture rebuild requires a passing clock check")
    environment_path = artifacts / "environment-lock.json"
    manifest_path = artifacts / "evidence-manifest.jsonl"
    if not environment_path.is_file() or not manifest_path.is_file():
        raise ValidationError("canonical environment-lock.json and evidence-manifest.jsonl are required")
    environment_raw = environment_path.read_bytes()
    environment = _load_canonical_json_bytes(environment_raw, "environment lock")
    validate_schema_instance(environment, load_json(SCHEMA_ROOT / "environment-lock.schema.json"))
    evidence_schema = load_json(SCHEMA_ROOT / "evidence-manifest.schema.json")
    normalized_schema = load_json(SCHEMA_ROOT / "normalized-records.schema.json")
    current_parser_hash = _sha256_file(PARSER_PATH)
    if environment["parser_artifact_sha256"] != current_parser_hash:
    ...
    for line_number, line in enumerate(manifest_lines, 1):
        ...
        if evidence_id in evidence_ids:
            raise ValidationError(f"duplicate evidence_id in manifest: {evidence_id}")
        evidence_ids.add(evidence_id)
        if evidence["environment_lock_sha256"] != environment_sha256 or evidence["parser_artifact_sha256"] != current_parser_hash:
            raise ValidationError(f"{evidence_id}: frozen environment/parser hash mismatch")
        raw_path = _resolve_canonical_ref(artifacts, evidence["stored_payload_path"], "raw")
        if not raw_path.is_file():
            raise ValidationError(f"{evidence_id}: referenced raw artifact is missing")
        raw_bytes = raw_path.read_bytes()
        ...
        if evidence_prefix not in {"p0-04", "p0-06"}:
            raise ValidationError(f"{evidence_id}: unsupported evidence prefix")
        fetch = FetchResult(
            requested_url=evidence["requested_url"], redirect_chain=tuple(evidence["redirect_chain"]),
            final_url=evidence["final_url"], retrieved_at=evidence["retrieved_at"], raw_body=raw_bytes,
            response_headers=tuple(evidence["response_headers"]), redacted_header_names=tuple(evidence["redacted_header_names"]),
        )
        with tempfile.TemporaryDirectory() as temporary:
            replay = process_capture(
                game=evidence["game"], issue_id=evidence["issue_id"], fetch=fetch,
        ...
        if replay.evidence != evidence:
            raise ValidationError(f"{evidence_id}: evidence replay differs from manifest")
        if replay.parse_result is not None:
            parsed_path = output_root / "parsed" / f"{evidence_prefix}-{evidence['game']}-{evidence['issue_id']}.json"
            _atomic_write_bytes(parsed_path, canonical_json_bytes(replay.parse_result) + b"\n")
            written.append(parsed_path)
        if replay.normalized is not None:
            validate_schema_instance(replay.normalized, normalized_schema)
            normalized_path = output_root / "normalized" / f"{evidence_prefix}-{evidence['game']}-{evidence['issue_id']}.json"
            _atomic_write_bytes(normalized_path, canonical_json_bytes(replay.normalized) + b"\n")
