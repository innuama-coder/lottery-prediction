...
跨 CPython 3.12 patch 或平台重算的 numeric contract 采用 `P4-LOCAL-PATH-CLASSIFIED-BINARY64-5`。完整 r11 macOS CPython 3.12.11 replay（124,880 numeric comparisons）推翻单个首错驱动的 17-ULP 假设：163 个旧 bound failures 包含 31-ULP shadow display probability、151-ULP nested model-context F04 与 15-ULP coefficient。`18b25e21`/`eb9c124d` 保留为该不完整假设的失败证据。随后 c5a9 全量 preflight 证明 coefficient 与 probability profiles 已关闭，但仍有 17 failures：nested `number_features` absolute 最大 `4 * 2^-53`，`top_zone_rows.*.0` 最大 `4 * 2^-53`、relative `1.1285989447903035e-14`、64 ULP。

新合同按计算来源分六类，且每类仍为 finite + absolute/relative/ULP conjunction：`derived_feature_context_v2` 只覆盖 feature snapshot 的 42 个既有 patterns 与两个 context normalization patterns，保持 `3 * 2^-53/3e-14/151`；nested `number_features` 的单一 pattern 进入 `derived_number_feature_context_v1`，使用其 observed absolute maximum `4 * 2^-53` 并保持 `3e-14/151`；`derived_coefficient_v1` 与 `top1000_derived_probability_display_v3` 保持 c5a9 已通过的 `1e-12/1e-12/16` 与 `2^-71/(17 / 2^52)/32`；单一 `top_zone_rows.*.0` pattern 进入 `propagated_zone_score_v1`，absolute 使用 observed `4 * 2^-53`、relative 使用高于 observed maximum 的最小 binary power `2^-46`、ULP 使用 observed maximum 64；其余 35 个 tight patterns 原样保持 `1e-12/1e-12/8`。controller preflight 必须输出每个 concrete pattern 的 comparison/difference/failure count 与三轴 maxima，并在任一新 bound failure、未分类 numeric leaf 或未观察 configured pattern 时非零退出。
...
稳定 key contract 与 path-classified numeric contract 均属于 D00 冻结 authority，而不是实现细节：key ID、quantum、rounding、identity namespace、复合排名键、numeric path set 或 profile bound 任一变化都必须升级显式版本并重新经过 authority freeze。
