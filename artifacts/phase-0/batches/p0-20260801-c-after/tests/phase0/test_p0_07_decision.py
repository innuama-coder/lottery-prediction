...
from archive_p0_c_pre import (  # noqa: E402
    historical_after_archive_root, historical_repair_draft_path,
    verify_archive, verify_archived_repair_draft,
)
...
class P007DecisionTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        after_archive = historical_after_archive_root(REPO)
        archived_draft = historical_repair_draft_path(REPO)
        pre_snapshot = verify_archive(REPO, check_current=False)
        cls.repair = verify_archived_repair_draft(
            REPO,
            archived_draft,
            pre_manifest_sha256=sha256_file(pre_manifest),
            pre_snapshot=pre_snapshot,
            after_archive_root=after_archive,
        )
