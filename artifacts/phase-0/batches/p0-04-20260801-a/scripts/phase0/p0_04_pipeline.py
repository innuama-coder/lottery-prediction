...
def verify_captures(artifacts: Path, clock_check: ClockCheck) -> int:
    ...
    for line_number, line in enumerate(manifest_lines, 1):
        ...
        if not raw_path.is_file() or not normalized_path.is_file():
            raise ValidationError(f"{evidence_id}: referenced raw or normalized artifact is missing")
        ...
        normalized = _load_canonical_json_bytes(normalized_raw, f"{evidence_id} normalized record")
        ...
        parsed_path = artifacts / "parsed" / f"p0-04-{evidence['game']}-{evidence['issue_id']}.json"
        if not parsed_path.is_file():
            raise ValidationError(f"{evidence_id}: parser trace is missing")
