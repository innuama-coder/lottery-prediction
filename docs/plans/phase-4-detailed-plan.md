...
版本：2.4（path-classified portable verifier preflight authority）
...
- **实现方法：** 独立解析 Phase 1、重算 F01–F14、训练目标/选择、组合分数归一和 Top-1000；重算所有 hash/parent 关系，不导入产品核心特征或概率实现。正式 builder provenance 继续 byte/hash exact；本地 CPython 3.12 verifier 使用 `P4-LOCAL-PATH-CLASSIFIED-BINARY64-5`。按来源分别路由 35-pattern tight (`1e-12/1e-12/8`)、44-pattern feature/context (`3*2^-53/3e-14/151`)、1-pattern nested number-feature context (`4*2^-53/3e-14/151`)、2-pattern coefficient (`1e-12/1e-12/16`)、1-pattern propagated zone score (`4*2^-53/2^-46/64`) 与 3-pattern display probability (`2^-71/(17/2^52)/32`) profiles；全部 finite 且三界 conjunction。score/tie identity 则由 `P4S10HE1`（exact-decimal rational、`1e-10`、half-even）唯一派生并 exact 比较，排名使用 stable key desc 后接 canonical ticket asc。
...
- **验收方法：** clean 隔离目录 replay；分别突变一条早期 draw、cutoff、rolling/EWMA/gap、pair/结构 feature、系数、model ID、概率、Top-1000 顺序、stable score key、tie/score identity、model lineage、lock 和 CLI provider 引用；加入六个 controller 一-ULP score fixtures、完整 r10 六 scope/6,000 行稳定键检查，以及完整 r11 双 game/full replay collector。collector 在保留 exact fail-fast 的同时汇总全部 numeric comparisons，按 concrete path pattern 输出 count/difference/failure 与 absolute/relative/ULP maxima；Linux migration 必须零新 failures，macOS CPython 3.12.11 controller preflight 也必须零新 failures，且 just-outside 每一 profile 的三轴边界与非有限负例必须失败。删除/tamper final closure 必须失败。
- **Pre-allocation HOLD：** c5a9 macOS collector 已覆盖全部 86 patterns，coefficient 与 probability profiles 为零 failures；剩余 1 个 nested context absolute failure 与 16 个 propagated zone-score failures 必须由各自单一 pattern profile 在下一次完整 macOS preflight 中关闭。此前禁止分配 successor release。
