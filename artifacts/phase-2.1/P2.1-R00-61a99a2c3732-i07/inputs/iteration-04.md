...
- 迭代编号：`04`
- 新 release ID：`P2.1-R00-60d02be4dbe9-i04`
...
第四轮必须基于远端 worktree 中第三轮留下的未提交修改继续，不得覆盖、删除或回滚这些修改。先定位并修复超时根因，再完成原第三轮所有阻断问题。禁止仅通过扩大超时时间掩盖慢路径；可以拆分 E2E staging、缓存只读基准证据、降低负向用例重复重算、增加超时保护和进度 receipt，但不得削弱最终验收的真实性。
...
必须在同一新 bundle `P2.1-R00-60d02be4dbe9-i04` 上完成：
